...
```{r}
str(Indometh)
#not our typical data frame format, but we can still work with it. Let's make it easier...
...
```{r}
library(ggplot2)
ggplot(Indometh, aes(x=Subject, y=conc)) 

```


Why didn't that work? Because we need to specify layers for the ggplot. Let's try a bar plot: 

```{r}
ggplot(Indometh, aes(x=time)) + geom_bar(stat="count")

```

Now let's add some additional layers of visual information and formatting like colors and themes. 

```{r}
ggplot(Indometh, aes(x=time)) + geom_bar(stat="count", fill="blue")


ggplot(Indometh, aes(x=time)) + geom_bar(stat="count", fill="blue")+theme_bw()
```

### So how does the concentration change over time?

```{r}
ggplot(I, aes(x=time, y=conc, group=Subject)) + geom_boxplot()
```

What if I wanted to see all the points? Stack geom_plots on top of each other:

```{r}
ggplot(Indometh, aes(x=Subject, y=conc)) + geom_boxplot()+ geom_point()
```

Now, what if we wanted to look at what is happening to the Indometh concentration over time within each of the subjects? 

We can add col = Variable and plot the points!

```{r}
ggplot(Indometh, aes(x=Subject, y=conc, col = time)) + geom_boxplot() + geom_point()
# col = color
...
```{r}
median(Indometh[Indometh$Subject==1,]$conc)
```

Is there a cleaner way? Don't be afraid to google how others are doing things. You may find a package you have never heard of that does EXACTLY what you want!

Try: install.packages("dplyr"). This has a very nice filter function that makes the question easy!

```{r}
library(dplyr)

Sub1 <- filter(Indometh, Indometh$Subject==1)
